...
if __name__ == '__main__':
    ...
    postfix = '_%d_%d'%(begin, end)
    tem_path = '/home/qustliu/AST/data/pde/ast%s' % postfix
    if not os.path.exists(tem_path):
        os.makedirs(tem_path)
    try:
        time_stamp = 0 

        difftokens = json.load(open('/home/qustliu/AST/data/pde/difftoken.json'))
        processed_tokens = pickle.load(open('/home/qustliu/AST/data/pde/processed_tokens.pkl', 'rb'))
        processed_types = pickle.load(open('/home/qustliu/AST/data/pde/processed_types.pkl', 'rb'))
        Flag = json.load(open('/home/qustliu/AST/data/pde/Flag.json'))
        diffmsg = json.load(open('/home/qustliu/AST/data/pde/diffmsg.json'))
        difftext = json.load(open('/home/qustliu/AST/data/pde/difftext.json'))
        ...
        for m, lines_tokens in enumerate(tqdm(processed_tokens[begin:end])):
            ...
            for k, tokens in enumerate(lines_tokens):
                ...
                if types[k] == 100:  
                    ...
                    code_old = tokens[0]
                    code_new = tokens[1]
                    all_token_new += code_new
                    all_token_old += code_old
            ...
            all_root_old, all_root_old_pos_old = get_ast(all_token_old, 'old_all_%d'%index)
            all_root_new, all_root_old_pos_new = get_ast(all_token_new, 'new_all_%d'%index)
            index += 1
        ...
        output_path = '/home/qustliu/AST/data/DataSet_%s'%postfix
