...
def get_ast_root(file_name, tem_path):
    out = subprocess.Popen('/home/qustliu/AST/gumtree/gumtree/bin/gumtree parse %s/%s.java'%(tem_path, file_name), shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
...
def get_ast_action(file_name1, file_name2, root1, root2, tem_path):
    out = subprocess.Popen('/home/qustliu/AST/gumtree/gumtree/bin/gumtree diff %s/%s.java %s/%s.java'%(tem_path, file_name1, tem_path, file_name2), shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    stdout, _ = out.communicate()
